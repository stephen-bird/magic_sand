...
gravel_std <- expA_15min_sub$gravel_rate / expA_15min_sub$gravel_feed
gravel_std <- ifelse(is.finite(gravel_std)==TRUE,gravel_std,NA)
expA_15min_sub$gravel_std <- gravel_std

sand_std <- expA_15min_sub$sand_rate / expA_15min_sub$sand_feed
sand_std <- ifelse(is.nan(sand_std)==FALSE,sand_std,NA)
expA_15min_sub$sand_std <- sand_std

good <- complete.cases(expA_15min_sub)
expA_15min_sub <- expA_15min_sub[good,]

plot(expA_15min_sub$time,expA_15min_sub$gravel_std,type="b")
plot(expA_15min_sub$time,expA_15min_sub$sand_std,type="b")

expA_dat_n <- select(expA_15min_sub,gravel_std,sand_std)
...
s_expA_recon <- reconstruct(s_expA, groups = list(1)) #list the eigenvectors that represent the trend -- "s_expA_recon" contains the residuals
...
seasons_expA <- reconstruct(s2_expA, groups = list(1:2,5:6))
parestimate(s2_expA, groups = list(1:2,5:6), method = "esprit")
plot(seasons_expA)
...
recon_expA <- reconstruct(s_expA,
                          groups = list(Trend = 1, Seasonality = c(2:3,6:7)))
