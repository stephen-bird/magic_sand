...
expI_dat <- expI_15min_sub[,c(2,4)]
norm_expI_dat <- sqrt(colMeans(expI_dat^2,na.rm=T))
expI_dat_n <- sweep(expI_dat, 2, norm_expI_dat, "/")
...
s_expI_recon <- reconstruct(s_expI, groups = list(1:2)) #list the eigenvectors that represent the trend -- "s_expI_recon" contains the residuals
...
seasons_expI <- reconstruct(s2_expI, groups = list(1:2,3:4,5:6))
parestimate(s2_expI, groups = list(1:2,3:4,5:6),method = "esprit")
plot(seasons_expI)
...
recon_expI <- reconstruct(s_expI,
                     groups = list(Trend = 1:2, Seasonality = c(3:4,5:6,7:8)))
